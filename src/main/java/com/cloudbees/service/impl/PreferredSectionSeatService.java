...
@Service("preferredSectionSeatService")
public class PreferredSectionSeatService implements SeatAssignment {
    ...
    public Seat assignSeat(Train train, Section section) {
        List<Seat> allSeats = train.getAllSeatsFromSection(section);
        synchronized (allSeats) {
            ...
            if(availableSeatOption.isEmpty()) {
                throw new InvalidInputException("No seat available in Section");
            }
